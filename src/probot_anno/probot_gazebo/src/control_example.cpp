...
int main(int argc, char **argv) {
    ...
    float x[4] = {0.2, 0.2, 0.15, 0.3};
    float y[4] = {0.2, 0.2, 0.2, 0};
    float z[4] = {0.2007, 0.2007, 0.2007, 0.122};
    float alpha[4] = {PI/2, PI/4, 0, PI/2};
    float beta[4] = {0, -PI/3, 0, 0};
    float gamma[4] = {0, 0, 0, 0};
...
if (group_num == 0) continue;
    ...
    target_pose.position.x = x[group_num-1];
    target_pose.position.y = y[group_num-1];
    target_pose.position.z = z[group_num-1]-0.022;
		

    float sa = sin(alpha[group_num-1]/2 + PI/4);
    float ca = cos(alpha[group_num-1]/2 + PI/4);
    float sb = sin(beta[group_num-1]/2);
    float cb = cos(beta[group_num-1]/2);
    float sg = sin(gamma[group_num-1]/2);
    float cg = cos(gamma[group_num-1]/2);

    target_pose.orientation.w = ca*cb*cg + sa*sb*sg;
    target_pose.orientation.x = sa*cb*cg - ca*sb*sg;
    target_pose.orientation.y = ca*sb*cg + sa*cb*sg;
    target_pose.orientation.z = ca*cb*sg - sa*sb*cg;
    ...
    //创建用于保存逆解的变量，它是vector矩阵，vector<std::vector<double>>类型，因为会解出不止一组解装在
//里面
